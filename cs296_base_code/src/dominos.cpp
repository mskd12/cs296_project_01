	 ...
	 {//pistonSphere	 
		...
		for(int i=0; i<5; i++) {
			...
			
			{
				...
				b2BodyDef bd; 
				groundBody[i] = m_world->CreateBody(&bd); 
				groundBody[i]->CreateFixture(&groundBodyShape, 0.0f);
			}
		   ...
		   {//left piston side
				...
				wall_left = m_world->CreateBody(&bd);
				wall_left->CreateFixture(&fd);
			}
			{//right piston side
				...
				wall_right = m_world->CreateBody(&bd);
				wall_right->CreateFixture(&fd);
			}
			{//top piston side
				  b2Vec2 vertices[3];
				  ...
				  wall_top = m_world->CreateBody(&myBodyDef);
				  wall_top->SetUserData(new int(20 + i));
				  wall_top->CreateFixture(&myFixtureDef); //add a fixture to the body
			...
			{//Fuel control(have added the control rod )
				  ...
				  prismaticJointDef.Initialize(fuelControl[i], groundBody[i], fuelControl[i]->GetWorldCenter(), v);
			...
			{//Exhaust control(have added the control rod )
				  ...
				  prismaticJointDef.Initialize(exhaustControl[i], groundBody[i], exhaustControl[i]->GetWorldCenter(), v);
				  m_world->CreateJoint(&prismaticJointDef);
			...
			{//fuel plank. This controls fuel intake
				b2PolygonShape polygonShape;
				...
				myBodyDef.type=b2_dynamicBody;
				myBodyDef.angle = (230 - 36*i)*degtorad+angle;
			...
			{//exhaust plank .This controls outgoing gas at exhaust
				b2PolygonShape polygonShape;
				polygonShape.SetAsBox(2.5, 0.1);
				b2FixtureDef myFixtureDef;
					b2BodyDef myBodyDef;
			...
			{//testing with small balls. Here we make the 3 walls and the gas particles(circles) inside the walls
				b2PolygonShape polygonShape;
				...
				fuelIntakeLeft->CreateFixture(&myFixtureDef); 
				...
				myFixtureDef1.shape = &polygonShape1;
				...
				fuelIntakeRight->SetUserData(new int(60 + i));
				{
					...
					myFixtureDef1.filter.categoryBits = 0x0004;
					myFixtureDef1.filter.maskBits = 0x0005;
					myBodyDef1.position=transform(b2Vec2(15,61));
					b2Body* Body1 = m_world->CreateBody(&myBodyDef1);
					Body1->CreateFixture(&myFixtureDef1); 
				}
			...
			{//piston, pistonRod and some joints related to them are defined here
				{
					b2Vec2 vertices[4];
				  ...
				  shape.Set(vertices, 4);
					...
					piston[i]->CreateFixture(&myFixtureDef1); 
				}
				
				{
					...
					pistonRod->CreateFixture(&myFixtureDef1); 
				}
				b2RevoluteJointDef* revolutejd = new b2RevoluteJointDef();
				...
				jointGndSphereDef->bodyA = groundBody[i];
				...
				m_world->CreateJoint(jointGndSphereDef);

				b2PrismaticJointDef prismaticJointDef;
				b2Vec2 v;
				v.Set(cos(90*degtorad+angle),sin(90*degtorad+angle));
				prismaticJointDef.Initialize(piston[i], groundBody[i], piston[i]->GetWorldCenter(), v);
				m_world->CreateJoint(&prismaticJointDef);
...
//Here we override the step function of base_sim_t class
void dominos_t::step(settings_t* settings){
		base_sim_t::step(settings);
	//creating new balls in place of ones which have gone out	
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < nBalls; j++) {
			if (ball_array[j][i] != NULL) {
				...
				if (ball_rad > 70 || ball_rad < 35) {
					...
					ball_array[j][i]->SetLinearVelocity(b2Vec2((rand() % 10)-2.5f, (rand() % 10) - 2.5f));
					ball_array[j][i]->GetFixtureList()->SetDensity( 0.0000014f);
					ball_array[j][i]->SetUserData(new int(40 + i));
				}
					...
					while(contactEdge != NULL) {
						...
						if (a != NULL && b != NULL && *a == 40 + i && *b == 50 + i) {
							bodyA->GetFixtureList()->SetDensity(0.00056f);
							bodyA->SetUserData(new int(50 + i));
						}
						if (a != NULL && b != NULL && *a == 50 + i && *b == 40 + i) {
							bodyB->GetFixtureList()->SetDensity(0.00056f);
