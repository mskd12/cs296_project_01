...
//category 4 and 5 used
   ...
   {//left piston side
	...
	{//fuel part
		...
		myBodyDef.type=b2_dynamicBody;
		myBodyDef.angle = (20)*degtorad+angle;
	...
	{//exhaust part
		...
		myBodyDef.type=b2_dynamicBody;
		myBodyDef.angle = (-160 + 72*i)*degtorad+angle;
		myFixtureDef.shape = &polygonShape;
		myFixtureDef.density = 1;
		myFixtureDef.restitution = 0.1;
	...
	for (int i = 0; i < 5; i++) {
		...
		for (int j = 0; j < nBalls; j++) {
			if (ball_array[j][i] != NULL) {
				...
				if (ball_rad > 70 || ball_rad < 28) {
					angle = 72*degtorad*i;
					x = 0;
					y = 0;
					ball_array[j][i]->SetTransform(transform(b2Vec2((rand() % 2) + 12.0f, (rand() % 2) + 57.0f)), 0);
					ball_array[j][i]->SetLinearVelocity(b2Vec2((rand() % 10) - 5.0f, (rand() % 10) - 5.0f));
					ball_array[j][i]->GetFixtureList()->SetDensity( 0.0000007f);
					ball_array[j][i]->SetUserData(new int(40 + i));
				}
